...
#Categorise the scoring variables 
...
age <- as.integer(age)
age <- na.omit(age)
age_mod <- case_when(age %in% 1.0:49.9 ~ 0,  #age_mod
                     age %in% 50.0:59.9 ~ 1,
                     age %in% 60.0:69.9 ~ 2,
                     age %in% 70.0:79.9 ~ 3,
                     age %in% 80.0:100.0 ~ 4)
age_mod <- as.factor(age_mod)
table(age_mod)

#Recalcuate CCI as numbers are not correct 

Trauma_data$cci2 <- as.integer(age_mod) + as.integer(Trauma_data$IHD) + as.integer(Trauma_data$CHF) + as.integer(Trauma_data$`Peripheral vascular disease`) +
  as.integer(Trauma_data$`CVA/ TIA`) + as.integer(Trauma_data$Dementia...94) + as.integer(Trauma_data$Dementia...96) + 
  as.integer(Trauma_data$COPD) + as.integer(Trauma_data$`Connective tisse`) + as.integer(Trauma_data$`Peptic ulcers`)+
  as.integer(Trauma_data$`Liver disease`) + as.integer(Trauma_data$T1DM) + as.integer(Trauma_data$T2DM) +
  as.integer(Trauma_data$hemiplegia) + as.integer(Trauma_data$CKD) + as.integer(Trauma_data$Tumour)+
  as.integer(Trauma_data$leukemia) + as.integer(Trauma_data$lymphoma) + as.integer(Trauma_data$AIDS)

Trauma_data <- Trauma_data[Trauma_data$cci2,] #CCI2 data
Trauma_data <- Trauma_data[complete.cases(Trauma_data$cci2),] #remove cases without CCI data
cci2 <- Trauma_data$cci2
cci2

#Export edited CCI2 dataset 

write.csv(Trauma_data,"~/Downloads/traumadata.csv", row.names = FALSE)


#CCI2 median survivors vs non-survivors 

summary(cci2)
summary(Trauma_data$CCI)

survivors <- Trauma_data[Trauma_data$Dead == '0',]
non_survivors <- Trauma_data[Trauma_data$Dead == '1',]

summary(survivors$cci2)
summary(non_survivors$cci2)

#Modify CCI removing age component 

cci_mod <- as.integer(cci2) - as.integer(age_mod)
...
tiff("Fig1.tiff", res = 300)
mech_counts <- table(mech)
barplot(mech_counts, main="Mechanism of Injury", horiz=TRUE, xlim = c(0, 200), cex.names=0.8, xlab = "Number of patients", ylab = "Mechanism of injury")
dev.off()
...
barplot(most_severe_injury_loc_counts, main="Most severely injured location", horiz = TRUE, xlab = "Number of patients", ylab = "Most severely injured location")
...
univariate <- glm(dead ~ cci2, family = binomial (link=logit) )
summary(univariate)
...
Trauma_data_multivariate <- Trauma_data[, c("Age", "GCS (Ps calculation)",
                                            "ISS", "Ps", "APACHE", "Number of Operations", "cci2",
...
Trauma_data_multivariate$Age[Trauma_data_multivariate$Age<50] <- "<50"
Trauma_data_multivariate$Age[Trauma_data_multivariate$Age>50 & Trauma_data_multivariate$Age <60] <- "50-60"
Trauma_data_multivariate$Age[Trauma_data_multivariate$Age>60 & Trauma_data_multivariate$Age <70] <- "60-70"
Trauma_data_multivariate$Age[Trauma_data_multivariate$Age>70 & Trauma_data_multivariate$Age <80] <- "70-80"
Trauma_data_multivariate$Age[Trauma_data_multivariate$Age>80] <- ">80"

age <- as.numeric(Trauma_data$Age)  
age # Age is recoded 
...
age_mod <- case_when(age %in% 1.0:49.9 ~ 0,  #age_mod
                     ...
                     age %in% 80.0:100.0 ~ 4)
age_mod
age_mod <- integer(age_mod)
age_mod
cci_mod <- as.integer(cci2) - as.integer(age_mod)
cci_mod

#Add CCI 
cci2 <- Trauma_data_multivariate$cci2

multivariate <- glm(dead ~ mech+age_mod+GCS+num_op+cci_mod, family = binomial (link=logit) )
summary(multivariate)
exp(multivariate$coefficients)
exp(confint(multivariate))

multivariate <- glm(dead ~ mech+age+GCS+num_op+cci_mod, family = binomial (link=logit) )
summary(multivariate)
exp(multivariate$coefficients)
exp(confint(multivariate))

univariate <- glm(dead ~ age, family = binomial)
summary(univariate)
exp(univariate$coefficients)
exp(confint(univariate))
...
univariate6 <- glm(dead ~ PS, family = binomial, data = Trauma_data)
...
plot.roc(dead, univariate5$fitted.values, percent = TRUE, col = "forestgreen", lwd = 1, add = TRUE, print.auc = FALSE, print.auc.y = 40, print.auc.x = 40)
plot.roc(dead, univariate6$fitted.values, percent = TRUE, col = "brown", lwd = 1, add = TRUE, print.auc = FALSE, print.auc.y = 30, print.auc.x = 40)


legend("bottomright", title = "Predictors of mortality", 
       legend = c("Age", "APACHE II score", "ISS", "GCS", "Number of surgeries", "Ps19"), col = c("black", "blue", "red", "orange", "forestgreen", "brown"), 
       lwd = 2, "Legend", cex=0.8)


#Revisit fall 

Trauma_data_falls <- Trauma_data[Trauma_data$Mechanism == "Fall less than 2m" | Trauma_data$Mechanism == "Fall more than 2m", ] #Filter for falls 
...
Comparison_table <-
  tbl_summary(
    Trauma_data_falls,
    include = c("Age","CCI", "GCS (Ps calculation)", "Most severely injured body region", "Dead"),
    by = "Mechanism", # split table by group
...
Trauma_data_dead <- Trauma_data[Trauma_data$Dead == 1,] #Filter for dead

table(Trauma_data_dead$Mechanism)

Trauma_data_alive <- Trauma_data[Trauma_data$Dead == 0,] #Filter for alive

table(Trauma_data_alive$Mechanism)

Comparison_table_deadvsalive <-
  tbl_summary(
    Trauma_data,
    include = c("Age","Sex...4", "CCI", "Mechanism", "GCS (Ps calculation)", "Most severely injured body region",
                "ISS", "Ps", "APACHE", "Number of Operations", "Most severely injured body region"),
    by = "Dead", # split table by group
    missing = "no" # don't list missing data separately
  ) %>%
  add_n() %>% # add column with total number of non-missing observations
  add_p() %>% # test for a difference between groups
